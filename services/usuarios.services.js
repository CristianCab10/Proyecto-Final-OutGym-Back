...
const recuperarPassUserServices = async (emailUsuario) => {
    try {
        ...
        if (usuarioExiste) {
            ...
            const tokenRecuperarPass = jwt.sign(payload, process.env.JWT_SECRET_RECOVERY_PASS)
...
const cambioPassUserTokenServices = async (token, newPass) => {
    try {
        const verificarUser = jwt.verify(token, process.env.JWT_SECRET_RECOVERY_PASS)
        const usuario = await UsuariosModel.findOne({_id: verificarUser.idUsuario})
